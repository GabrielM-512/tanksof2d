...
def pos_after_rot(point, angle, pivot):
    ...
    return [
        x - pivot[0] * math.cos(angle_rad) + pivot[0],
        y - pivot[1] * math.sin(angle_rad) + pivot[1]
    ]
...
class Tank:
    def __init__(self):
        ...
        self.base_chassis_connect_point = (127, 150)
    ...
    def draw(self, screen):
        ...
        old_pos = self.icon.get_rect().center
        ...
        new_pos = self.icon.get_rect().center

        diff = (old_pos[0] - new_pos[0], old_pos[1] - new_pos[1])

        self.offsetx += diff[0]
        self.offsety += diff[1]

        self.chassis_connection_point = pos_after_rot((self.base_chassis_connect_point[0] - self.offsetx, self.base_chassis_connect_point[1] - self.offsety), self.chassis_angle, (128, 128))
        self.chassis_connection_point[0] -= diff[0]
        self.chassis_connection_point[1] -= diff[1]

        print(self.chassis_connection_point)

        screen.blit(self.icon, (self.hitbox.x + self.offsetx, self.hitbox.y + self.offsety))
        pygame.draw.rect(screen, (255, 0, 0), ((self.chassis_connection_point[0] + self.hitbox.x, self.chassis_connection_point[1] + self.hitbox.y), (2, 2)))
        pygame.draw.rect(screen, (0, 255, 0), (self.hitbox.x + 128, self.hitbox.y + 128, 2, 2))
